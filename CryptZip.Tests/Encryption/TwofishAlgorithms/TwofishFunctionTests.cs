using CryptZip.Encryption.TwofishAlgorithms;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace CryptZip.Tests.Encryption.TwofishAlgorithms
{
    [TestClass]
    public class TwofishFunctionTests
    {
        [TestMethod]
        public void h_CalculateshValue_Calculated()
        {
            uint expected = 579114735;
            uint result = TwofishFunction.h(new TwofishMDS(), 261617515, new uint[] {2715, 515187, 1251512, 215815});
            Assert.AreEqual(expected, result);
        }

        [TestMethod]
        public void q0_Calculatesq0Value_Calculated()
        {
            #region expectedValue
            byte[] expected =
            {
                
            };
            #endregion

            for (int i = 0; i < 256; i++)
                Assert.AreEqual(expected[i], TwofishFunction.q0(i));
        }

        [TestMethod]
        public void q1_Calculatesq1Value_Calculated()
        {
            #region expectedValue
            byte[] expected =
            {
                
            };
            #endregion

            for (int i = 0; i < 256; i++)
                Assert.AreEqual(expected[i], TwofishFunction.q1(i));
        }
    }
}
